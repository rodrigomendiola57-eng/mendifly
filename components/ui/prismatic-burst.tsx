...
function toPx(v?: number | string | null): number {
  if (v == null) return 0;
...
function getBlendMode(
  mode: CSSProperties["mixBlendMode"] | "none" | undefined,
): string {
...
export function PrismaticBurst({
  ...
  useEffect(() => {
    ...
    // Verify WebGL2 support before attempting to initialize OGL
    const testCanvas = document.createElement("canvas");
    const webgl2 = testCanvas.getContext("webgl2");
    if (!webgl2) return; // silently degrade — no WebGL2 available
    ...
    try {
      ...
      return; // WebGL init failed — degrade silently
    }
    ...
    gl.canvas.style.mixBlendMode = getBlendMode(mixBlendMode);
    ...
    try {
      ...
      // Shader compilation or mesh setup failed — degrade silently
  ...
  return (
    <div
      ref={containerRef}
      aria-hidden
      className={cn("prismatic-burst-container pointer-events-none", className)}
    />
  );
}
