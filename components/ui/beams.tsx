...
function extendMaterial(
  ...
  uniforms["diffuse"] = { value: new THREE.Color(0, 0, 0) };
...
function createStackedPlanesGeometry(
  ...
  for (let i = 0; i < n; i++) {
    ...
    const uvXOffset = ((i + 1) * 47.3) % 300;
    const uvYOffset = ((i + 1) * 19.7) % 300;
...
function BeamsScene({
  ...
  const beamMaterial = useMemo(
    () => extendMaterial(speed, noiseIntensity, scale),
...
export function Beams(props: BeamsProps) {
  ...
  // Use "active" only to drive frameloop: "always" | "never".
  // The Canvas is always mounted so R3F can measure the container correctly
  // from the very first render — no timing race with layout animations.
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      ...
      // Start animating 200 px before the section enters the viewport
      { rootMargin: "200px" },
    );
    io.observe(el);
  ...
  return (
    ...
    >
      {/* Canvas is always in the DOM so R3F reads correct dimensions on mount.
          frameloop="never" keeps the GPU idle when the section is offscreen. */}
      <Canvas
        dpr={dpr}
